...
// multiply p_output_freq by 2 so that it goes through a full cycle instead of half a cycle
localparam p_count =        (p_input_freq + p_output_freq) / (p_output_freq * 2);
localparam p_count_size =   $clog2(p_count);

reg [p_count_size-1:0]  r_count = {p_count_size{1'b0}};
reg                     r_clk   = 1'b0;

always @(posedge i_clk) begin
    if (~i_rst) begin
        r_count <= {p_count_size{1'b0}};
        r_clk   <= 1'b0;
    end
    else begin
