module lcd_controller (i_clk, ar, wind_data, wind_direction, new_data, o_lcd_rs, o_lcd_rw, o_lcd_e, o_lcd_db, init_done, done_writing);
	...
	output o_lcd_rs;				//RS pin to clarify writing data or instructions
	output o_lcd_rw;				//RW pin to switch between read and write
	output o_lcd_e;				//Enable pin to initiate writes
	output [7:0] o_lcd_db;		//DB pin data written to the LCD
	...
	//States to pulse o_lcd_e during writes
	...
	assign o_lcd_db = init_done ? write_data : init_data;
	assign o_lcd_e = init_done ? LCD_e_write : LCD_e_init;
	assign o_lcd_rs = init_done ? LCD_rs_write : LCD_rs_init;
	
	assign o_lcd_rw = 1'b0;	//Set RW to write only
    ...
    reg r_prev_new_data;
    wire w_rising_new_data = ~r_prev_new_data && new_data;
		...
		if(~ar)
			...
			case(cs1)
				Idle1:
					...
					if(w_rising_new_data && init_done)
						begin
